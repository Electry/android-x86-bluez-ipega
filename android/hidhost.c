...
/* ipega pg-9025 descriptor override */
static unsigned char ipega_data[289] = {

};

/*
static void dump_memory(void *data, size_t len)
{
	size_t i;
	error("Data in [%p..%p): ", data, data+len);
	for (i = 0; i < len; i++)
		error("%02X", ((unsigned char*)data)[i]);
}
*/
...
static int uhid_create(struct hid_device *dev)
{
	...
	error("UHID: bus=0x%x", BUS_BLUETOOTH);
	error("UHID: vendor=0x%x", dev->vendor);
	error("UHID: product=0x%x", dev->product);
	error("UHID: version=0x%x", dev->version);
	error("UHID: rd_size=%d", dev->rd_size);

	// ipega bad firmware hack
	if (dev->vendor == 0x5ac && dev->product == 0x22c) {
		strcpy((char *) ev.u.create.name,
			"ipega-gamepad-controller");
		ev.u.create.vendor = 0x1949;
		ev.u.create.product = 0x0402;
		if (dev->rd_size == 0) {
			ev.u.create.rd_size = sizeof(ipega_data);
			ev.u.create.rd_data = malloc(sizeof(ipega_data));
			memcpy(ev.u.create.rd_data, &ipega_data,
				sizeof(ipega_data));

			error("UHID: new rd_size=%d",
				ev.u.create.rd_size);
		}
	}
	//dump_memory(ev.u.create.rd_data, dev->rd_size);
